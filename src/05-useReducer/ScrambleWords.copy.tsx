...
export const ScrambleWords = () => {
  ...
  const handleSkip = () => {
    if (maxSkips === skipCounter) return;
    const updatedWors = words.slice(1);
    setSkipCounter(prev => prev + 1);
    setWords(updatedWors);
    setCurrentWord(updatedWors[0]);
    setScrambledWord(scrambleWord(updatedWors[0]));
